...
//Stores the values from that bout of travel into a history list
struct Node* FloorHistory(struct Node* visits, struct Node* refinedUser)
{
	struct Node* temp = refinedUser;
	
	while (temp != NULL)
	{
		append(&visits, temp->data);
		temp = temp->next;
	}
...
int main()
{
	//Create empty doubly linked lists
	struct Node* floors = NULL;
	struct Node* visits = NULL;
	...
	do
	{
		...
		//Places desired floors into a list, sort them by value, then place them into a history list
		struct Node* rawUser = AssignFloors(floors, numOfFloors);
		struct Node* refinedUser = OrganizeFloors(rawUser);
		visits = FloorHistory(visits, refinedUser);
	...
	printf("History of floors traveled to: ");
	print_list(visits);
